...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors.filterNot {
    neighbor => explored.contains(neighbor._1)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val next = for {
      item <- initial
      neighbor <- newNeighborsOnly( neighborsWithHistory(item._1, item._2), explored)
    } yield neighbor
    
    if (next.isEmpty) initial
    else {
