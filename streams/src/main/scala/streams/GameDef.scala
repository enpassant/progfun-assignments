...
trait GameDef {
  ...
  case object Left  extends Move {
    override def apply(block: Block): Block = block.left
  }
  case object Right extends Move {
    override def apply(block: Block): Block = block.right
  }
  case object Up    extends Move {
    override def apply(block: Block): Block = block.up
  }
  case object Down  extends Move {
    override def apply(block: Block): Block = block.down
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    } yield (move(this), move)

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter(tuple =>
      tuple._1.isLegal
    )
