...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def loop(ts: TweetSet): TweetList = ts match {
      case _ :Empty => Nil
      case _ => 
        val tw = ts.mostRetweeted
        new Cons(tw, loop(ts.remove(tw)))
    }
    loop(this)
  }
...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def loop(ts: TweetSet, tw: Tweet): Tweet = ts match {
      case ts: Empty => tw
  	  case nets: NonEmpty => {
  	    loop(nets.left, loop(nets.right, if (tw.retweets > nets.elem.retweets) tw else nets.elem))
  	  }
    }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter { tweet =>
    google.exists(tweet.text.contains)
  }
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter { tweet =>
    apple.exists(tweet.text.contains)
  }
  ...
  lazy val mostRetweeted = appleTweets.union(googleTweets).mostRetweeted
}

object Main extends App {
  println(GoogleVsApple.mostRetweeted)
